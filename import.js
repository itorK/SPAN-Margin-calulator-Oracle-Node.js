...
var download = function(url, dest, cb) {
  ...
  oracle.connect(connectData, function(err, connection) {
    ...
    function DodajSpready(fhandler1) {
      ...
      for (k = 0; k < klasy.length; k++) {
        ...
        for (i = 0; i < nodes.length; i++) {
          ...
          for (j = 0; j < v_nogi.length; j++) {
            ...
            connection.execute(
                    "call pk_import.prDodajSpread(:1,:2,:3,:4,:5,:6,:7)",
                    ['W', v_depozyt, v_priorytet, v_klasa, v_poziom, v_strona, v_delta],
        ...
        for (i = 0; i < v_poziomy.length; i++) {
          ...
          if (v_pocz.length == 6 && v_kon.length == 6 && v_poziom_klasy != 99) {
            connection.execute(
                    "UPDATE span_papiery SET sppa_data_gen = TO_TIMESTAMP(:1,'YYYYMMDDHH24MI'), sppa_nr_poziomu = :2 WHERE sppa_data_wygas BETWEEN TO_DATE(:3,'YYYYMM') AND TO_DATE(:4,'YYYYMM') AND sppa_klas_id  = (SELECT klas_id FROM klasy WHERE nvl(substr(klas_nazwa_span,1,INSTR(klas_nazwa_span,'/')-1),klas_nazwa_span) = :5) ",
                    [v_data_gen,v_poziom_klasy, v_pocz, v_kon, v_klasa],
                    function(err, results) {
                      if (err) {
                        console.error(err);
                        console.log(v_klasa);
      ...
      for (i = 0; i < nodes.length; i++) {
        ...
        for (j = 0; j < v_nogi.length; j++) {
          ...
          connection.execute(
                  "call pk_import.prDodajSpread(:1,:2,:3,:4,:5,:6,:7)",
                  ['Z', v_depozyt, v_priorytet, v_klasa, v_poziom, v_strona, v_delta],
                  function(err, results) {
                    if (err) {
                      console.error(err);
                      return;
    ...
    function DodajPapier(fhandler, cb) {
      ...
      stmt = "BEGIN pk_import.prCzysc(); pk_span.prCzysc(); ";
      for (i = 0; i < nodes.length; i++) {
        v_klas_id = 0;
        ...
        for (j = 0; j < v_serie.length; j++) {
          ...
          stmt = stmt + " pk_import.prDodajPapier('" + v_sppa_nazwa + "','" + v_kod_klasy + "','" + v_data_wyg + "'," + v_kurs_instr + ",'" + v_typ_papieru + "'," + v_czas_do_wygas + "," + v_psr + "," + v_ryz_zmien + "," + v_mnoznik + "," + v_delta_ref + "," + v_stopa_proc + "," + v_wsp_skal + "," + v_cena_instr + "," + v_scen1 + "," + v_scen2 + "," + v_scen3 + "," + v_scen4 + "," + v_scen5 + "," + v_scen6 + "," + v_scen7 + "," + v_scen8 + "," + v_scen9 + "," + v_scen10 + "," + v_scen11 + "," + v_scen12 + "," + v_scen13 + "," + v_scen14 + "," + v_scen15 + "," + v_scen16 + ",'" + v_kod_span + "'); ";
        }
      ...
      for (i = 0; i < nodes.length; i++) {
        ...
        for (j = 0; j < v_serie.length; j++) {
          ...
          for (l = 0; l < v_opcje.length; l++) {
            ...
            stmt = stmt + " pk_import.prDodajPapierO('" + v_sppa_nazwa + "','" + v_kod_klasy + "','" + v_data_wyg + "'," + v_kurs_instr + ",'" + v_typ_papieru + "'," + v_czas_do_wygas + "," + v_psr + "," + v_ryz_zmien + "," + v_mnoznik + "," + v_delta_ref + "," + v_stopa_proc + "," + v_wsp_skal + "," + v_cena_instr + "," + v_scen1 + "," + v_scen2 + "," + v_scen3 + "," + v_scen4 + "," + v_scen5 + "," + v_scen6 + "," + v_scen7 + "," + v_scen8 + "," + v_scen9 + "," + v_scen10 + "," + v_scen11 + "," + v_scen12 + "," + v_scen13 + "," + v_scen14 + "," + v_scen15 + "," + v_scen16 + ",'" + v_kod_span + "'," + v_zmien_op_wygasl + "," + v_stopa_dyw + "," + v_kurs_wyk + ",'" + v_rodzaj_opcji + "'," + v_zmien_op + "," + v_cena_baz + "); ";

          }
        }
      }

      stmt = stmt + " COMMIT; END;"
      connection.execute(
              stmt, [],
              function(err, results) {
                if (err) {
